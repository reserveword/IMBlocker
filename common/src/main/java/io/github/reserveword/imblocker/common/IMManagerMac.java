...
final class IMManagerMac implements IMManager.PlatformIMManager {
	...
	private static final UnmarkTextCallback UnmarkTextImp;
	private static final UnmarkTextCallback NewUnmarkTextImp;
	...
	static {
		...
		// unmarkText replacement \\
		Pointer unmarkTextSelector = RuntimeUtils.sel("unmarkText");
		Pointer unmarkTextMethod = Runtime.INSTANCE.class_getInstanceMethod(viewClass, unmarkTextSelector);
		UnmarkTextImp = getImp(UnmarkTextCallback.class, unmarkTextMethod);
		NewUnmarkTextImp = (self, selector) -> {
			UnmarkTextImp.invoke(self, selector);
			postPreeditContent(null, 0);
		};
		ObjC.INSTANCE.class_replaceMethod(viewClass, unmarkTextSelector, NewUnmarkTextImp, "v@:@");
	...
	private interface KeyDownCallback extends Callback {
		void invoke(Pointer self, Pointer selector, Pointer event);
	}
	...
	private interface UnmarkTextCallback extends Callback {
		void invoke(Pointer self, Pointer selector);
	}
	...
	private static <T extends Callback> T getImp(Class<T> callbackClass, Pointer method) {
		return (T) CallbackReference.getCallback(callbackClass, ObjC.INSTANCE.method_getImplementation(method));
	}
