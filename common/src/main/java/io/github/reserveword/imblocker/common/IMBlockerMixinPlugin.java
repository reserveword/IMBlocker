	...
	static {
		if(isClientSide()) {
			...
			if(IMBlockerCore.isGameVersionReached(771/*1.21.6*/)) {
				validMixins.add("IMEOverlayRendererV3");
			}else if(IMBlockerCore.isGameVersionReached(763/*1.20*/)) {
				validMixins.add("IMEOverlayRendererV2");
			}else if(IMBlockerCore.isGameVersionReached(755/*1.17*/)) {
...
//				validMixins.add("compat.GeneralFeedbackTextareaMixin"); Unstable Internal Implementation.
	...
	private static boolean isClientSide() {
		...
		for(String className : distClasses) {
			try {
				Object dist = Class.forName(className).getMethod("getDist").invoke(null);
		...
		try {
			...
			Object environmentType = fabricLoader.getMethod("getEnvironmentType").invoke(instance);
			return "CLIENT".equals(String.valueOf(environmentType));
		} catch (ReflectiveOperationException ignored) {}
